...
async def discover_account(domain: str, user_id: str, supabase: Client):
    """
    Menjalankan workflow lengkap Scout AI dan menyimpannya ke database dengan konteks RLS pengguna.
    """
    # 1. Search (Tavily)
    raw_data = await search_company_info(domain)
    
    # 2. Analyze (Gemini)
    analysis = analyze_company(domain, raw_data)
